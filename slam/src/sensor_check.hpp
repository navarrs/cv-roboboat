...
public:
    static rs2::device get_a_realsense_device()
    {
        ...
        if (devices.size() == 0)
        {
            std::cerr << "[ERROR] No device connected. Quitting program." << std::endl;
            exit(EXIT_FAILURE);
        }
        else
        {
            std::cout << "[INFO] Found the following devices:\n" << std::endl;
            ...
            {
                throw std::out_of_range("[ERROR] Selected device index is out of range");
            }
